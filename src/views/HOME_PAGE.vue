...
const toggleFavorite = (livroId) => {
  favorite.value = favorite.value.includes(livroId)
    ? favorite.value.filter(id => id !== livroId)
    : [...favorite.value, livroId]
}

const isFavorite = (livroId) => favorite.value.includes(livroId)

watch(favorite, (val) => {
  localStorage.setItem('favoriteBooks', JSON.stringify(val))
}, { deep: true })

onMounted(() => {
  const stored = localStorage.getItem('favoriteBooks')
  if (stored) favorite.value = JSON.parse(stored)
})
</script>
  ...
  <main id="home">
    <section id="offer">
      <div id="offer-text">
        ...
        <p>Eric-Emmanuel Schmitt has been awarded more than 20 literary prizes and distinctions, and in 2001 he received
    ...
    <section id="releases">
      ...
      <ul>
        <li v-for="livro in livros" :key="livro.id">
          ...
          <div id="space-div">
            <p>{{ livro.preco }}</p>
            <span class="mdi mdi-heart" :style="{ color: isFavorite(livro.id) ? 'red' : '#27ae60' }"
              @click="toggleFavorite(livro.id)"></span>
          </div>
